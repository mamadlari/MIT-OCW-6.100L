...
class TestPS4BC(unittest.TestCase):
    ...
    def test_plaintext_message_inheritance(self):
        ...
        for plaintext, text in zip((self.ptmsg1, self.ptmsg2, self.ptmsg3, self.ptmsg4),
            ...
            self.assertEqual(text, plaintext.get_text(), f"The PlaintextMessage({text})'s get_text() returns {
                             plaintext.get_text()} not {text}. Are you calling the super constructor?")
    ...
    def test_encrypted_message_inheritance(self):
        for text in (self.text1, self.text2, self.text3, self.text4):
            ...
            self.assertEqual(text, encrypted.get_text(), f"The EncryptedMessage({text})'s get_text() returns {
                             encrypted.get_text()} not {text}. Are you calling the super constructor?")
